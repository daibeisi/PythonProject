...
class ExamplePublisher:
    ...
    这是一个例子，发布者将处理与RabbitMQ的意外交互，如通道和连接闭包。如果RabbitMQ关闭连接，
    它会重新打开它。您应该查看输出，因为连接可能关闭的原因有限，通常与权限相关的问题或套接字超时有关。
    它使用了传递确认，并演示了一种跟踪已经发送的消息以及它们是否已经被RabbitMQ确认的方法。
    """
    EXCHANGE = 'message'
    ...
    def __init__(self, amqp_url):
        ...
        设置发布者的例子对象，传入我们将用来连接RabbitMQ的URL。
    ...
    def connect(self):
        ...
        这个方法连接到RabbitMQ，返回连接句柄。当连接建立后，pika将调用on_connection_open方法。
    ...
    def on_connection_open(self, _unused_connection):
        ...
        一旦建立了与RabbitMQ的连接，pika就会调用这个方法。
        它将句柄传递给连接对象，以备需要，但在本例中，我们将其标记为未使用。
    ...
    def on_connection_closed(self, _unused_connection, reason):
        ...
        当与RabbitMQ的连接意外关闭时，pika会调用这个方法。因为这是意料之外的，
        所以如果RabbitMQ断开，我们将重新连接它。
